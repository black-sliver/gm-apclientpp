...
GMS2+ is (currently) not because it uses different syntax for strings.
...
* `ap_room_update(len: int)` called when locations for the connected slot were checked, len is the amount of locations.
   Use `global.arg_ids: int[]` to retrieve the checked locations.
